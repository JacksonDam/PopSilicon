...
for Bejeweled 2). Dropping a recognised game app selects it, and its product,
...
the build starts Steam if it is not already running. The two Bejeweled Steam
copies are not DRM-wrapped, so they install without Steam running. (Bejeweled
2's carries an extra `__STEAM` segment holding Valve's ownership stub, but the
game code beside it is in the clear; the loader skips that stub, which reads a
Mac OS X 10.4 dyld's private globals and cannot run here.) After confirmation,
...
Deluxe, `Peggle Nights` for Nights, `Bejeweled3` for Bejeweled 3, `Bejeweled 2`
for Bejeweled 2). An older install whose game image is still the encrypted
...
BASS; the runtime serves its calls from the same BASS build.) The first build
runs `tools/fetch_bass.py`, which downloads the
